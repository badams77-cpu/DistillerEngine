...
public class Corpora implements Serializable {
	...
	final static long serialVersionUID = 0xBABEBABE00000001L;
	
	public Hashtable<String,Hashtable<String,Integer>> langWords;
	public Hashtable<String,Integer> totals;
	...
	private void addParsedFile(Parser parser, String lang){
		Integer tot = totals.get(lang);
		if (tot==null){ tot = new Integer(0); }
		...
		totals.put(lang,new Integer(total));
	}
...
//     Remove all elements with occurance=1
		...
		for(Enumeration<String> langE=totals.keys();langE.hasMoreElements(); ){
			...
			int total = totals.get(lang).intValue();
